...
class Arm:
    def __init__(self, port_handler, packet_handler):
        ...
        self.motor_list = [Motor(1, port_handler, packet_handler, 355, 760, 100, 1, 32, 32),
                           Motor(2, port_handler, packet_handler, 206, 660, 100, 1, 16, 16, 0),
    ...
    def get_xy(self):
        t1, t2, t3, t4 = self.get_thetas()
        s, c = np.sin, np.cos
        return -9.75*s(t3)*c(t1+t2)-5*s(t4)*s(t1+t2)+22*c(t1)+5*c(t3)*c(t4)*c(t1+t2)+7*c(t3)*c(t1+t2)+24*c(t1 + t2)\
            , 22*s(t1)- 9.75*s(t3)*s(t1+t2)+5*s(t4)*c(t1+t2)+5*s(t1+t2)*c(t3)*c(t4)+7*s(t1+t2)*c(t3)+24.5*s(t1 + t2)
