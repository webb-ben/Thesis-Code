...
def visualizeArm(t0, z0=None, ax=None, ep=None, fig=None):
    ...
    if ax == None:
        ...
        ep = fig.add_subplot(212, aspect="equal")
    ...
    for i in range(1, t0.shape[0]):
        l.extend(ax.plot([t0[i - 1][0][3], t0[i][0][3]], [t0[i - 1][1][3], t0[i][1][3]], [t0[i - 1][2][3], t0[i][2][3]],
                         'o-',
    ...
    return np.mat([[-np.sin(t4) * np.sin(t1 + t2) + np.cos(t3) * np.cos(t4) * np.cos(t1 + t2),
                   ...
                   [np.sin(t4) * np.cos(t1 + t2) + np.sin(t1 + t2) * np.cos(t3) * np.cos(t4),
                    ...
                    22.3 * np.sin(t1) - 18.5 * np.sin(t3) * np.sin(t1 + t2) + 6 * np.sin(t4) * np.cos(
                        t1 + t2) + 6 * np.sin(t1 + t2) * np.cos(t3) * np.cos(t4) + 6.5 * np.sin(t1 + t2) * np.cos(
                        t3) + 25.3 * np.sin(t1 + t2)],
...
def inverse_jacobian(a1, a2, a3, a4, x, y, z, Y):
    jac = np.linalg.inv(
        np.mat([[-22.3 * s(a1) - 25.3 * s(a1 + a2) - 6.5 * s(a1 + a2) * c(a3) + 17 * s(a1 + a2) * s(a3) - 6 * s(
...
def jacobian(a1, a2, a3, a4, t1, t2, t3, t4):
    return (np.mat([[-22.3 * s(a1) - 25.3 * s(a1 + a2) - 6.5 * s(a1 + a2) * c(a3) + 17 * s(a1 + a2) * s(a3) - 6 * s(
        a1 + a2) * c(a3) * c(a4) - 6 * c(a1 + a2) * s(a4),
                     -25.3 * s(a1 + a2) - 6.5 * s(a1 + a2) * c(a3) + 17 * s(a1 + a2) * s(a3) - 6 * s(a1 + a2) * c(
                         a3) * c(
                         a4) - c(a1 + a2) * s(a4),
                     -6.5 * c(a1 + a2) * s(a3) - 17 * c(a1 + a2) * c(a3) - 6 * c(a1 + a2) * s(a3) * c(a4),
                     -6 * c(a1 + a2) * c(a3) * s(a4) - 6 * s(a1 + a2) * c(a4)],
                    [22.3 * c(a1) + 25.3 * s(a1 + a2) + 6.5 * c(a1 + a2) * c(a3) - 17 * c(a1 + a2) * s(a3) + 6 * c(
                        a1 + a2) * c(a3) * c(a4) - 6 * s(a1 + a2) * s(a4),
                     25.3 * s(a1 + a2) + 6.5 * c(a1 + a2) * c(a3) - 17 * c(a1 + a2) * s(a3) + 6 * c(a1 + a2) * c(
                         a3) * c(
                         a4) - 6 * s(a1 + a2) * s(a4),
                     -6.5 * c(a1 + a2) * s(a3) - 17 * c(a1 + a2) * c(a3) - 6 * c(a1 + a2) * c(a3) * c(a4),
                     -6 * c(a1 + a2) * c(a3) * s(a4) + 6 * c(a1 + a2) * c(a4)],
...
def step(dx, dy, dz, dhp, z0, ax, fig, ep):
    steps = 100
    a1, a2, a3, a4 = dhp[:4, 3]
    step_x = dx / (steps)
    step_y = dy / (steps)
    for j in range(steps):
        ...
        for i in range(dhp.shape[0]):
            ...
            t0[i] = np.mat([(c(dhp[i, 3]), -s(dhp[i, 3]), 0, dhp[i, 1]),
                            (s(dhp[i, 3]) * c(dhp[i, 0]), c(dhp[i, 3]) * c(dhp[i, 0]), -s(dhp[i, 0]),
                             -dhp[i, 2] * s(dhp[i, 0])),
                            (s(dhp[i, 3]) * s(dhp[i, 0]), c(dhp[i, 3]) * s(dhp[i, 0]), c(dhp[i, 0]),
                             dhp[i, 2] * c(dhp[i, 0])),
                            (0, 0, 0, 1)])
...
def main(argv):
    ...
    #     # a2 += (0.0008*step_x) + (1.55274*step_x) - (0.52867 * step_x)
    #     a1 += 0.046884 * step_x
    #     a2 += -1.009377 * 0.046884 * step_x - 0.5627 * 0.046884 * step_x * a1
    #     dhp[:, 3] = (np.mat([a1,a2, a3, a4, 0])).T
    #
